...
The rest are places where the same work was written twice and the two copies drifted. An install spawning `php:rebuild` had its own child mistake the install for a pending upgrade and start a second one from inside it. Enabling autostart armed every unit on disk, the tray included, so a tray switched off came back at the next login. Restoring a worker unit on `lerd start` rebuilt its command without the dev server setup the normal start applies, and the tool came back on its own port with no generated config. Assistant commands built the container name from the PHP version, so a site served from an image it builds for itself had artisan, composer and vendor binaries sent to the shared container instead.

### Fixed

- **A package check answered from composer.json rather than from what composer installed** (#1663). The lookup behind `check: composer:` gates read the require and require-dev maps and nothing else, so a package the project never named itself read as absent, and a package another one stands in for read as absent everywhere, since composer writes one entry for the package doing the replacing. `tempest/framework` replaces `tempest/database` and `tempest/command-bus`, so a stock Tempest project was offered neither the migration setup step nor the command monitor worker, and the same shape covers a distribution that ships drush or a skeleton that pulls messenger in underneath. The lookup reads composer.lock as well now, cached by modification time and size the way the manifest already is, counting every name in packages and packages-dev together with the keys of each replace and provide map. A project with no lock answers from the manifest exactly as before, so a scaffold that has not been installed yet behaves as it did. Detection keeps the manifest to itself, because it asks what the project is rather than what is installed under it: a library repo testing against Laravel carries `laravel/framework` in its lock without being a Laravel site.
- **A framework major came from the declared constraint rather than from the lock** (#1665). Three places said the version came from composer.lock as the source of truth and none of them opened it. What ran was a walk of composer.json that read the major off the first run of digits in the constraint, so a project written as `^11.0 || ^12.0` was served the Laravel 11 definition however new the release it runs, and a constraint of `dev-main` or `*` offered no digits at all and fell through to the version file and the pin. The lock answers first now for the framework's own composer package, with the manifest as the fallback for a project that has never been installed, and the store's second copy of the same walk becomes a delegation to that one lookup, so `lerd link`, `lerd framework list` and the assistant paths all place a project on the same major.
- **Assistant commands ran in the shared PHP container instead of the site's own** (#1660). A site with a Containerfile is served from an image built for it alone, and the CLI resolves that container before every exec, but the MCP server built the container name from the PHP version. Artisan, console, composer, vendor binaries and the framework setup steps an assistant ran went to the shared per-version container, which has none of the layers the project added, so a site whose image registers an ODBC driver or installs a binary the code shells out to failed on the first command that touched it while the identical command from the shell worked. The container rule lives next to the version rule now and both the CLI and the MCP server call it, and the reverb, horizon and scheduler units an assistant writes bind to and exec into the per-site container too.
- **`lerd install` reapplied itself through its own `php:rebuild`** (#1668). The install spawns `php:rebuild` when the FPM images need rebuilding, and the child took its own install for a pending upgrade: the version stamp that tells a binary whether its environment has been applied is written at the end of the install, so mid-install there is nothing to read, and on a first install or a reinstall the child concluded a package manager had swapped the binary underneath it. It then re-execed a complete `lerd install --from-update` before running the rebuild it was called for, which cost a second full install pass and two extra rounds of PHP image builds. The install marks its process tree now and the reapply bails when it sees the mark, which keeps the decision on the install itself rather than on the timing of a stamp file, and holds for every helper the install shells out to. Both upgrade paths are unchanged, and the daemons the install starts are exempt, so nothing inherits a flag that would suppress a genuine upgrade later.
- **Enabling autostart re-armed a system tray that had been turned off** (#1672). Turning the tray off disables `lerd-tray.service` so the applet stays gone at the next graphical login, but enabling autostart afterwards, from the CLI or from the dashboard toggle, armed every lerd unit found on disk and the tray unit was in that set regardless of the tray preference. The wants symlink came back and the applet returned at the following login with nothing in the session having asked for it. The login set passes through a filter that drops the tray unit while the preference is off, so the tray switch wins over the autostart one. Disabling autostart still covers every unit, the tray included, and turning the tray back on re-enables it the way it always did.
- **The bug report carried nothing a site declares for itself** (#1629). The report held the global config and the site registry, but the workers list, the custom worker definitions, the worker options a project has answered and the declared services all live in the per-site `.lerd.yaml`, so a report showing a worker unit sitting there with nothing running gave no way to tell a lost declaration from a start that failed. Each linked site's file prints under the registry now, framed and anonymized the same way the global config already is, and a site that never got one says so in a single line rather than printing an empty block.
- **A restored worker unit lost its dev server flags** (#1685). Starting lerd rewrites the worker units listed in a site config, and that path built the command without the dev server setup the normal worker start applies. A unit that is not armed for boot gets rewritten on every start, so the tool came back listening on its own port with no generated config, and the page then asked for its assets on an address the browser refuses to load from over https. Toggling the worker afterwards went through the full path and put the flags back, which is why it looked intermittent. The setup lives in one helper that both the worker start and the restore call, so a restored unit points at the same generated config and pinned port as one started by hand.

## [1.34.1] - 2026-09-02

A patch for NativePHP's Jump, which could not work on any Linux or macOS install. Jump serves a development build to a phone over the network, and its websocket bridge runs on Workerman, which calls posix and pcntl unguarded. The PHP that ships in nativephp/php-bin carries neither, on both platforms, so the bridge died on its first line inside a log file while the HTTP side kept answering: the phone loaded the landing page, found nothing to connect to, and dropped the session a few seconds later, which reads like a network problem and is not one. The binary is a static build with dynamic loading compiled out, so the extensions cannot be added to it on the machine either.

The fix is in the layer that already knows a command has to leave the container. A host command declaration can name the extensions the binary it points at has to carry, lerd asks that binary rather than assuming, and when it comes up short the command runs with lerd's own PHP instead. The declaration itself ships through the framework store, so the fix arms itself on an install that never upgrades, and an older binary ignores the new field and behaves exactly as it did.

### Fixed

- **NativePHP's Jump could not start its websocket bridge** (#1656). A host command declaration can now name the PHP extensions the binary it points at has to carry, and when that binary is short one the command runs with lerd's own PHP: a pinned static build, matched to the project's PHP minor because the command runs project code against a composer.lock resolved for that version, downloaded to the bin directory beside mkcert and fnm the first time a project needs one and never before. The download says what it weighs before it starts and spins a step line while it arrives, is verified against a sha256 pinned per platform, and falls back to the copy already installed when a moved pin cannot be fetched. It installs as php-host-&lt;minor&gt; rather than php, since php in that directory is the shim into the container. Because lerd asks the binary rather than assuming, the day upstream ships a fuller build the command goes back to the bundled one on its own, and the rest of the native command namespace is untouched: those drive Gradle and xcodebuild, and the bundled binary has everything they need.
- **Ctrl+c on a command opened from the dashboard reported a crashed shell**. The shell those commands run in is not interactive, so it died on the same SIGINT as the command it was waiting on, and interrupting a long-running one left the terminal saying the shell had crashed instead of showing the command's own shutdown and the pause that holds the window open. The script traps the signal now, as a handler rather than an ignore, since an ignored signal is inherited by every child and the command itself would stop answering ctrl+c.
- **A command opened from the dashboard did not get lerd's own bin directory on PATH**. `lerd run` and the dashboard's own runner both put it there before running anything, but a terminal command was spawned with the graphical session keys merged over whatever the service manager had imported, so a declaration naming php or lerd worked on one machine and failed as not found on the next.

## [1.34.0] - 2026-09-02

The theme of this release is that a worker stopped being something the binary knows about. Almost every worker and command lerd ships is gated on a composer package rather than on the framework carrying it, and it had to be written again into every major of every framework that could require it: `laravel/horizon` sat in eight Laravel files, `drush/drush` in four Drupal ones. The store gains a package layer, one file per composer package declaring the workers, commands, setup steps and doctor checks that belong to the package, merged onto whatever definition a project resolved. In the same cycle `queue:start`, `schedule:start`, `reverb:start` and `horizon:start` stopped being hand-written commands and are generated from the workers the project's framework declares, which took the last of the Laravel knowledge out of Go: the composer hints, the fixed queue flags and the redis preflight are all store data now. What a worker can be tuned with is declared the same way, and the values a project answers with live in its own `.lerd.yaml` rather than being retyped on every start.

The other half of the release is about getting into lerd without a terminal. Opening the dashboard on a stopped lerd left you looking at a page you could not act on, so the core services banner carries a Start button that streams the start back unit by unit, macOS installs a Lerd app with a splash that reads the same stream, and Linux gets a desktop entry that reports its cold start over the session bus. Opening a terminal from the dashboard now asks the desktop which terminal it is set to use, on both platforms, instead of walking a fixed list and losing to whatever happened to be on PATH. The tray becomes optional for desktops that already carry lerd's state somewhere else, the TUI gains the databases pane and the service facets that only the web UI had, and a command a project runs twenty times a day can be pinned to the site's control row instead of costing two clicks every time.

Bandwidth is the third thread. Several commands quietly committed to pulling or rebuilding container images, and PHP base images get republished often enough that a rebuild is a whole base download. Every command that can pull now says what it is about to fetch and roughly how big it is before the first byte moves, read off the registry manifest so working it out downloads nothing, with `--no-pull`, `LERD_OFFLINE=1` and `lerd start --dry-run` for the times you would rather not spend it. The dashboard turns the same estimate into a confirmation, and the MCP server answers with the image and its size instead of starting a download on a connection belonging to someone who never typed the command.

PHP 8.6 lands as a prerelease tier that no default ever picks, NativePHP is supported end to end on both the desktop and the mobile half, and `lerd doctor` now sweeps every linked site rather than reporting only on the host.

The fixed list covers bugs that shipped in 1.33.1 or earlier. Regressions introduced and fixed inside this cycle never reached a user, so they are folded into the feature that carried them rather than listed as fixes of their own. Three are listed anyway, because each was reported from a running install rather than caught in review: the extension clobber, where fixing ext/ftp in the base image only exposed how thoroughly a declared extension replaced it, the uninstall that emptied the data directory on its way to removing the launcher icon, and the doctor answering a re-check out of its own cache with the database it had just created still missing.

### Added

- **A composer package declares its workers once, not once per framework major** (#1574). Everything gated on a package had to be repeated in every version file of every framework that might require it, which is how `laravel/horizon` ended up written into eight Laravel definitions and how adding NativePHP meant putting the same worker, two commands and three checks into three files and then correcting all three twice in one afternoon. The framework store gains a package layer: one file per composer package, beside the definitions rather than inside them, declaring what belongs to the package rather than to whatever framework happens to carry it. A project that requires the package gets those declarations merged onto the definition it resolved, narrowed by an optional framework list and major range, so a queue driver stays unscoped while `nativephp/electron` says it is Laravel's alone. The package wins a name collision with a version file, which is what lets a declaration move without being shadowed by the copy it leaves behind, and a user overlay and a project's own `.lerd.yaml` still sit above both. A package carries versions the way a framework does: one that has kept its interface is a single file, and a major that changes what lerd runs adds a file serving that major and every later one, with what a major removes said out loud, since the copy the declaration was lifted out of is still in the framework's version files and silence there means keep it. Only packages the store index lists are ever looked up, so a project's dependency list never turns into a request for a file that does not exist, and an install that cannot reach a version it has never fetched falls back to the newest cached file below it, so a store that gains a version while a machine is offline never costs that machine the worker it has been running for months. `lerd framework list` prints the layer underneath the definitions, every package the store publishes, what each declares, which file answers for the project you are standing in and whether that project requires it, reading the cache and never fetching.
- **The named worker commands are generated from the framework definition** (#1356). `queue:start`, `schedule:start`, `reverb:start` and `horizon:start` were each a hand-written cobra command that ended up doing what `lerd worker start` already does from the definition, so a worker added to the store had no start command of its own and could never get one without a binary release. They are generated now, over the one implementation behind `worker start` and `stop`, with the existing spellings kept exactly as they were. The flags on a start command come from the `tune_command` placeholders rather than a fixed queue, tries and timeout set, and each default is read back out of the plain command so the value is declared once, which means a framework that spells its queue differently gets exactly the flags it declares and a worker with a reload variant gets a reload toggle the same way. That takes the last of the Laravel knowledge out of Go: the composer require hints and the laravel.com links are gone, the queue flags are no longer fixed in the binary, and `lerd check` reads a worker's own exclude and conflict rules instead of looking for `laravel/horizon` by name. The redis preflight that stopped a queue on `QUEUE_CONNECTION=redis` starting without lerd-redis becomes a `requires_service` block in the store, so any worker can name a service it needs and the env condition that scopes it, and the generated unit orders after that service rather than racing it at boot. A project that is cloned but not linked yet still gets its commands, resolved from the framework named in `.lerd.yaml`, so they answer with the link hint rather than an unknown command error.
- **A project's worker options live in its `.lerd.yaml`** (#1569). A worker's tunable values already came from the store, as `tune_command` placeholders rendered into the framework's own syntax, but nothing kept them, so a project running several queues spelled them on every start and the dashboard toggle, the restore after a reinstall and a fresh clone all fell back to the definition's defaults. They live under `worker_options` now, keyed by worker then placeholder, and every start resolves the command through them, so the queues and limits a project committed are what runs wherever the worker is started from; passing a flag writes it there instead of tuning a single run. The dashboard offers the same knobs, a gear beside the toggle for any worker whose definition declares options, one field per option, prefilled with what the project committed and showing the definition's default as the placeholder, saving to the file and restarting a running worker. A value equal to the framework's default is not stored, so a later store change to that default still lands, and a value carrying whitespace is refused, since it is interpolated into the command the unit runs.
- **Container image downloads are disclosed before they start** (#1524). Several commands quietly committed to pulling or rebuilding images, which hurts on a metered connection, and the PHP base images are republished often enough that a rebuild re-downloads the whole base. Every command that can pull or build now says what it is about to fetch, how big it is and why, before the first byte moves, with sizes read from the registry manifest over the OCI distribution API so working them out downloads nothing and a registry that does not answer in time leaves the line without a number rather than holding the command up. The disclosure covers `start`, `install`, `fetch` and `php:rebuild`, the service install, update, migrate, rollback and reinstall paths, the implicit pull in the middle of a container start, and the FrankenPHP runtime switch. A new `--no-pull` flag, or `LERD_OFFLINE=1` for the dashboard, the watcher and the MCP server, skips pulls and rebuilds unless the image is missing outright, so refreshing something that already works never spends bandwidth, and `lerd start --dry-run` reports what a start would pull or rebuild and exits without starting anything. In the dashboard the same estimate backs a confirmation naming the image and roughly how much it will download, while an image already in the local store costs nothing to use and so still runs with no extra click.
- **PHP 8.6 while it is still a prerelease** (#1584). There was nowhere in lerd to run a project's test suite against the next PHP, and adding it as an ordinary version would have offered a beta to people who assume a version list means supported. A prerelease tier alongside the frozen legacy one decides everything that follows: the image builds from the `8.6-rc` tag the PHP Docker library publishes until release, a bare `lerd fetch` leaves it alone so nobody builds a beta they did not ask for, FrankenPHP does not offer it because it publishes released PHP only, and every place a version is picked marks it, so it is never chosen in the belief that it gets security updates. Release day is one edit: drop the version from that list and the upstream tag, the FrankenPHP set and the labelling all follow. 8.6 removed PEAR, so `pecl` is gone from the upstream image and every PECL install goes through one wrapper that falls back to the release tarball when it is missing; redis, imagick and mongodb build that way, while igbinary, pcov and xdebug do not compile against 8.6 yet, so the image drops them and nothing advertises them on that version. The xdebug step is tolerant like its neighbours now, so a third-party extension that stops building can no longer fail a whole image.
- **A shell in a PHP version's container, from the dashboard and from anywhere** (#1592). The dashboard could inspect a PHP version from every angle except the one that answers most questions about an image, which is what it actually holds, and `lerd shell` only worked from inside a linked project, so looking at a version no site uses yet meant finding a site on it first or reaching for podman by hand. The version detail leads with a terminal button that opens a session in that version's shared container, inactive while the container is stopped, and `lerd shell 8.4` works from anywhere for the same reason: it starts a stopped container, never offers to build a missing one, and with no project behind it opens on the container's own directory rather than being forced somewhere arbitrary.
- **A command can be pinned to the site's control row** (#1575). Every framework command sat behind the commands dropdown, so the one a project runs twenty times a day cost the same two clicks as the one it runs twice a year. A pinned command draws as its own button on the control row beside the version pickers and the doctor button, and clicking it runs exactly what the dropdown entry would, confirm gate and terminal spawn included. Pinning happens from the pin icon on each dropdown entry, two per site, enforced by the API rather than only by the UI because the row folds its secondary actions into a menu on a narrow panel and an unbounded set would land on that fold first. A definition can ship a command pinned with `pinned: true` for the one everyone on a project reaches for, and the user's own choice lives in the site registry keyed by command name and overrides that default, so a preference stays personal instead of turning into a diff every teammate carries.
- **lerd starts from the dashboard, from a macOS app and from the Linux application list** (#1529, #1536). Opening the dashboard on a stopped lerd left you looking at a page you could not act on, so the only way back was a terminal. The core services banner carries a Start Lerd button that runs the same start the CLI does and streams its progress back, counting units as they come up and naming the stage it is in rather than hanging on a request that takes the better part of a minute. On macOS, `lerd install` writes a Lerd app to `~/Applications` so the environment shows up in Launchpad and Finder, drawing a splash with the mark, the unit currently coming up and a progress bar reading the same stream, waiting for the start to report itself finished rather than for nginx to answer, since the vhost is up well before the databases behind it. Linux gets a desktop entry written by `lerd install` and removed by `lerd uninstall`, reporting its cold start as a notification rewritten in place over the session bus rather than through a dialog tool the desktop may not ship, and named Lerd on its own unless the desktop app is already listed, in which case it steps aside as Start Lerd rather than shadowing an entry that carries the `lerd://` association. The paths that lead to the dashboard now agree on what to do when nothing is serving it: `lerd dashboard` and the tray's Open Dashboard start lerd first, the offline page offers a Start button whenever the daemon still answers on its own port, and its retry probe asks nginx for the dashboard page instead of an API route the vhost does not proxy on every platform. `127.0.0.1:7073` is treated as a fallback rather than a second home, so a loopback session moves onto `lerd.localhost` as soon as nginx answers and asks for neither notification permission nor a push subscription while it waits, since both are per origin and settling on the port would leave two browsers subscribed.
- **The terminal the desktop is set to use is the terminal lerd opens** (#1535). Open in terminal walked a fixed list, so an emulator that merely happened to be on PATH won over the one the user had actually chosen, and Warp never appeared at all. On macOS the choice lives in LaunchServices as the handler for terminal shell scripts, which is what the system means by a default terminal, and on Linux the lookup asks the freedesktop launcher, then the distribution's alternatives link, then the terminal KDE or GNOME records, keeping the fixed list for an install where nothing was ever picked. A chosen terminal is launched with its own flags where lerd knows them, since the generic `cd` wrapper is not something every emulator accepts and `TERMINAL=kitty` failed on exactly that. `$TERMINAL` still leads, since an explicit override should outrank a system setting, and Warp joins the fallbacks ahead of iTerm2 and Terminal.app for anyone who installed it without making it the default.
- **The system tray is optional** (#1525). Desktops that already carry lerd's running state elsewhere, the Omarchy bar widget being the case that prompted this, got the applet as a second copy of the same information with no supported way to turn it off, and masking the unit did not work because start enabled it again or launched the helper directly. There is an on/off preference in the global config now, next to the icon style and defaulting to on, checked by both start and install before they touch the applet, and turning it off quits the running one and takes the unit out of the autostart set. It is reachable from all three places a user might look: `lerd tray on` and `lerd tray off`, a toggle in the dashboard's system settings, and an item in the tray menu itself, which is deliberately one way since a gone applet has no menu left to bring it back from.
- **A databases pane in the TUI** (#1362). The dashboard has a whole Databases page and the TUI had none of it, so someone watching sites and services in the terminal could not see that a database existed, how large it had grown or whether it had a snapshot to fall back on. The informative half is a tab of its own now: the left column lists every installed engine with the databases inside it, each with its size and snapshot count, and the detail pane names the engine, the size, the site that owns the database and the branch when it is a worktree's isolated one, then every snapshot with its timestamp, size and branch. Taking a snapshot fits the quick-action rule, since it adds a file and takes nothing away, so it runs under `n` the way starting a service runs under `s`, from the owning project's directory so the snapshot records the site and branch it belongs to. Restore, drop, import and export overwrite or destroy data, so they stay in the CLI and the pane says where to find them. Which site owns which database, across group secondaries, paired testing databases and worktree registrations, moves into a package both surfaces read from rather than being worked out twice.
- **A service's client tools, tuning and entities in the TUI** (#1363). The service detail pane showed the image, the state, what the service depends on, which sites use it and its env vars, while the dashboard showed three more facets of the same service and someone debugging in the terminal had to leave for the CLI to see any of them. All three read as rows in the pane now. The client tools are information plus one reversible toggle, since turning a shim on or off writes or removes a file in the bin dir and flipping it back undoes it exactly, so they sit alongside start, stop and restart under space; a tool another installed service owns is listed for context and says so. Tuning reports the settings actually in effect with comments dropped and points at the CLI for the edit, because a whole-file edit is not a quick action, and entities are listed and nothing more, so creating or dropping one stays where it was.
- **The TUI marks the services that expose a web dashboard** (#1364). The TUI could open a service's browser UI, but only once the service was selected and only if the user already knew it had one, so nothing on screen said which of the sixty-odd rows were worth pressing `O` on. The marker sits in the row's meta column beside the version and the pinned and custom tags rather than in a tab of its own, since the list already carries every service and a tab for a handful of links is the kind of clutter the scope guard warns about.
- **NativePHP projects, desktop and mobile** (#1545). Both halves are supported end to end: `nativephp/electron` running the desktop app as a host worker, and `nativephp/mobile` building onto an emulator, a simulator or a device, with a walkthrough covering the parts that catch people out because neither reports itself clearly, an Xcode shipping an SDK without the matching simulator runtime and a Gradle picking up whichever JDK leads PATH when `JAVA_HOME` is unset. The declarations arrive through the store's new package layer rather than being copied into three Laravel files. Several things had to become framework-agnostic to carry it. A doctor check can take the same gate rule workers and commands already use, so a check about an optional package is dropped rather than drawing a permanently green row on every project that never installed it. A worker declares whether its command outlives what it started, so a `native:serve` that ends with the window the user just closed is no longer reported as drift asking to be started again, which was never NativePHP's alone since vite ends with its dev server the same way. A start refuses when the command names a project-relative program that is not there and says which one, instead of launching a unit that dies on "Can't spawn program" and flaps every few seconds. And a host worker's teardown reaps the whole process group, since launchd signals only the job leader and a worker that hands off to a launcher was leaving the Electron window on screen throwing EPIPE dialogs once its parent's stdio had gone.
- **A framework or a package declares the console commands that have to leave the container** (#1651). A console command that opens a desktop window cannot run where lerd runs every other one. NativePHP's `native:run` is the case: it starts Electron and shows a window, and the PHP-FPM container has neither. Nobody could see it coming either, because upstream documents `php artisan native:run` and on a lerd machine `php` is the shim on PATH ahead of `/usr/bin`, so the documented command quietly re-entered the container, ran npm in there and died two seconds later saying only that the dev script failed. A definition can name which commands have to leave now, and what runs them: a list of an argument pattern and a binary, merged from the package layer the way workers and commands already are, matched against the arguments as typed so one declaration covers `lerd php artisan native:run` and `lerd artisan native:run` alike, with the first match winning so a package is never shadowed by the framework file it merges over. A declared binary that is not installed yet is an error naming the install command rather than a fall back into the container, since falling back is precisely the failure the declaration exists to prevent, and the re-routed command runs under the project's Node through the version manager because what it fronts shells out to npm. Nothing in the binary names a framework or a package, so anything whose runtime cannot live in a container gets the same treatment by declaring it, and the NativePHP page says the upstream commands can be typed exactly as they are documented.
- **`lerd doctor` sweeps the linked sites** (#1358). The environment doctor reported on the host and stopped there, so a machine where every check passed could still be serving nothing. It ends with a `[Sites]` section now: every linked site runs the cheap half of the site doctor and gets one line, the counts plus the first thing wrong, naming `lerd site:doctor <domain>` for the detail, while the framework command checks, composer audit, npm audit and the response-time lookup stay on the per-site command. One engine's database list is reused across the sweep rather than costing a container exec per site. Validating `.lerd.yaml` moves into the site doctor as its own check so every surface runs it, which makes `lerd check` a deprecated alias, and a worker declared in a project's own `custom_workers` counts as defined on a plain site rather than only on a custom-container one.
- **The doctor creates a missing database from the finding**. A site pointing at a MySQL or Postgres schema its engine does not hold was told to go and run `lerd db:create`, the one finding on the panel ending in a command to type somewhere else, while the button it did carry ran the framework's migrate command, which has nowhere to run until the database exists. The finding creates the database now, from the dashboard and from `lerd site:doctor --fix` alike, running as a host action in the engine's container beside installing a service the site declares and resolving the missing set again on the way in so it can only ever create what the check reported. A database that does not exist suppresses the framework migration check, so the migrate button returns on the re-check that follows and the two steps stay in the order they have to happen in.
- **The doctor checks that containers can resolve an internet name** (#1522, #1519). Every DNS check was about `.test` resolving on the host, and aardvark-dns answers container names and `.test` from its own records, so those keep working while every other lookup goes to the forwarders the lerd network was handed. A stale or unroutable forwarder there stayed invisible until composer or npm timed out mid-download with a clean bill of health and nothing to point at. The DNS section now asks the running nginx container to resolve the framework store's own hostname, taken from the store base URL so a self-hosted store is probed rather than a name the install never fetches, skipping when nginx is not running and capping the lookup on wall clock because the failure it exists to catch hangs rather than answers. The hint points at re-pointing the network at the current host resolvers and at the command that shows what they are set to.
- **A location-scope nginx override per site**. A site's `custom.d` override is included at the end of its server block, which means it can never win against anything the generated vhost sets inside a location: nginx resolves `fastcgi_param` and `proxy_set_header` per location and drops the inherited set the moment a location declares one of its own. The practical consequence was that the forwarded host wiring could not be changed for a single site, so a project served through a tunnel always got `SERVER_NAME` and `HTTP_HOST` rewritten to `X-Forwarded-Host` and a codebase comparing `SERVER_NAME` against its own configuration had to hand-edit a generated vhost the next regeneration wipes. Every vhost carries a second include, `custom.d/{domain}.location.conf`, placed at the very end of the block that actually serves the site, the php location on an FPM site and `location /` on a proxied one, so a directive repeated there lands after everything lerd sets. It is a first-class override rather than a file you are left to manage: the site's nginx modal is tabbed into Server block and Location, both with `nginx -t` validation, backups, restore and reset, the CLI takes `--location` on show, edit and reset, the MCP site tool takes a scope, and the file follows a primary domain rename, is seeded into a new worktree and is removed with it exactly like the server-scope one.
- **`lerd cpx`** (#1543). cpx is to Composer what npx is to npm, a way to run a command from any Composer package without adding it to the project, and routing it through lerd is what makes it run on the PHP version the site is registered on rather than whatever PHP the host happens to have. Most of this already worked: requiring cpx globally makes lerd sync a shim onto PATH like any composer global binary, and lerd execs with `HOME` set to the host's home so the packages cpx fetches cache under `~/.cpx` and survive a container restart. What the command adds is the explicit form and an error worth reading, since without it a missing cpx fails inside the container with "could not open input file" rather than naming the one command that fixes it.
- **The MCP server registers with OpenCode** (#1542). OpenCode is the one client so far that shares neither of the two JSON shapes lerd already writes: its entry lives under an `mcp` key rather than `mcpServers` or `servers`, names the transport local rather than stdio, folds the command and its arguments into a single array and expects the entry to say it is enabled. It also reads `opencode.json` and `opencode.jsonc` alike while asking for one format per directory, so an existing `.jsonc` is the file lerd merges into, with the shared JSON reader stripping comments so it parses at all, and the inject and eject lines name the file that was actually written rather than the one the client declares.
- **`lerd update` shows what changed once the update lands** (#1359). `lerd update` printed the changelog before asking for confirmation, then downloaded, installed, restarted services and returned without a word about what actually changed, while `doctor` and `status` both ended by pointing at `lerd whatsnew`. The same changelog is printed once the update is done, gated on an interactive run so an unattended or piped one does not dump it into a log. It is also a changelog again: `FetchChangelog` read `CHANGELOG.md` at the repo root, which is a symlink, and raw.githubusercontent serves the target as the body, so the parser had been seeing no version headings and coming back empty since April, which is why `lerd whatsnew` answered that it found no entries. The file is read at `docs/changelog.md` pinned to the tag that just installed, so what prints is the changelog of the version that landed, and both surfaces print a short list of each entry's headline with its issue numbers rather than nine thousand characters of prose twice in one run.
- **The installer can follow the beta line** (#1645). The version to fetch was resolved through the `releases/latest` redirect, which is defined to skip prereleases, so a published beta was invisible to `install.sh` and the only way onto that line was downloading the tarball by hand. `--beta` reads the newest tag off the releases atom feed instead, which is newest first and needs no API token, so the resolution stays free of a rate limit. It is a modifier rather than a command, taken out of the argument list before the dispatch, so it works on a first install and alongside `--update`, and since the feed carries stable releases too it lands on the stable one as soon as that overtakes the beta line. Telling a git-describe tail apart from a semver prerelease had to come with it, since a beta was read as a local development build and compared equal to the stable release of the same number, and someone on a beta who re-runs the plain installer is asked to confirm the downgrade rather than being taken back to stable without a word.
- **MCP: a worker's tunable values come off its framework definition** (#1617). The CLI renders one flag per `tune_command` placeholder, so a definition that makes another worker tunable arrives with its flags already there, while the MCP tool had queue, tries and timeout written into its schema by hand with only `queue_start` reading them. The worker list reports each worker's options now, the placeholder alongside the default the definition runs and the value the project committed, and start takes them back as `name=value` pairs handed to the same generated command the CLI renders, so there is one derivation rather than two. A name the worker does not declare is refused where it was passed rather than dropped on the way to a start that would look like it applied, and a value carrying whitespace is refused too since it lands in the unit's `ExecStart` line. `queue_start` and `queue_stop` stay as the queue worker's own spelling and now run the same path, which also gains them the worktree branch they never had.
- **MCP: the image an action would download is disclosed before it is fetched** (#1618). Every command that can pull says what it is about to fetch and the dashboard asks before spending the bandwidth, while an assistant installing a preset, updating a service or adding an extension started the same download with nothing said about it. The actions that can pull read the estimate off the registry manifest, which downloads nothing, and answer with the image and its size instead of fetching it, so the assistant can relay the cost and come back with `confirm: true` once the person paying for it has agreed. An image already in the local store is not worth interrupting for and neither is an estimate the registry did not answer, so both go straight through, and a pull that could not be sized is still disclosed without a number rather than quoting one it does not have.
- **MCP: `project_new` scaffolds the major it was asked for** (#1616). `lerd new` grew a way to scaffold an older major when it became a wizard, but the MCP tool behind the same operation resolved the definition with an empty version, so an assistant asked for a Laravel 11 project got whatever the store publishes as current and the mismatch surfaced later, in a `composer.json` nobody asked for. The tool takes the version the rest of the framework tool already takes, and since a major nothing publishes resolves to the current one, which is right for a person picking from a list and wrong for a caller that named one, it compares what came back against what was asked for and refuses the mismatch rather than scaffolding something else. A version with no framework beside it is refused too, since the default would otherwise absorb a major meant for another framework.

### Security

- **DOMPurify (GHSA-55q2-fjhq-7xh7)**. The dashboard's editor pulls DOMPurify in through monaco-editor, and 3.4.12 and earlier leave a detached subtree executable when an `IN_PLACE` sanitization hook removes an element, so a descendant image can keep an attacker's `onload` and fire it after `sanitize()` has returned clean. Nothing in the dashboard calls the sanitizer that way, so no shipped build was exploitable, but the pinned version moves to 3.4.14 so the vulnerable code is not in the bundle at all.

### Fixed

- **nginx never bound the ports it was configured with** (#1544). `nginx.http_port` and `nginx.https_port` were read by everything that talks to nginx, the share commands, the profiler probe, the watcher and the install-time port check, but never by the thing that actually binds: both writers rendered the bundled quadlet template, which publishes `80:80` and `443:443`, so on a host where another service owns those ports there was no supported way to move nginx off them. It looked macOS-only from the plist, but the launchd generator builds its `-p` arguments from the quadlet's `PublishPort` lines, so it was faithfully reproducing a template both platforms share. The host side of those mappings is rewritten by matching the container port, leaving 80 and 443 inside the container where nginx listens, both writers call it, and a start rewrites the unit and restarts the container when the mapping changed, since a running container keeps the ports it was created with. The plain-HTTP vhost also redirected to `https://$host` with no port, so on a host that had moved its HTTPS port every http visit landed on 443 with nothing listening; the redirect carries the configured port when it is not the default. Doctor's port section was the visible half of the same problem, probing the literal 80 and 443 and reporting them fine without probing at all when nginx was up, so a host with nginx demonstrably on 10080 was told about port 80 either way.
- **Every container exited on a host with IPv6 turned off** (#1634). Every service quadlet had a `[::1]` publish line paired onto its `127.0.0.1` one, gated only on the unit carrying a `Network=` key, and rootlessport cannot bind an address the kernel does not have. A host booted with `ipv6.disable=1`, or one whose VPN client turns IPv6 off for as long as it is connected, had podman fail that bind and treat the failure as fatal, so nginx, mysql, redis and everything else exited with status 126 and there was no working environment left to diagnose it from. Quadlet writes look for `::1` before they pair anything and reduce the publish lines to IPv4 when it is gone, dropping a v6 line whose v4 twin is already there and rewriting it to that twin when it is not. The check is deliberately weaker than the one steering the network schema, which wants a routable address, because a loopback publish only ever needs the loopback. Units written by an earlier build are repaired on the next start, which rewrites them and restarts whatever was already running, and the host is read every time, so a machine that regains IPv6 gets its dual stack pairs back with it.
- **An uninstall that reported keeping the data had already emptied it** (#1635). The Linux desktop entry's icon lives at `~/.local/share/lerd/lerd.png`, inside lerd's own data directory rather than an icon directory of its own, and the launcher removal cleared the icon's parent. That step runs whatever the answer to the data prompt was, so an uninstall that went on to say the data was kept had taken the site registry, the nginx vhosts, the certificates and the request stats, with nothing in the output connecting the two. Service data survived by accident alone, because containers write it as a subuid and the removal could not touch those files. Only the icon file goes now.
- **Re-running the installer flipped a `.localhost` machine onto managed DNS** (#1647). The DNS mode is settled once: `lerd install` samples whether a config already exists, honours the saved choice on every later run, and leaves the flip to `dns:enable` and `dns:disable`. `install.sh` did not know that, asking the question on every run and always passing the answer on as an explicit flag, which is the one input that wins ahead of the saved choice, so the rule that says never ask again was defeated by the installer sitting on top of it. Where there was no terminal the question was not even shown, since it reads from `/dev/tty`, and the answer silently resolved to managed: a machine deliberately set up on `*.localhost` was converted, dnsmasq came up, the resolver sudoers grant was written and every site domain was rewritten, on nothing more than a reinstall. Only a first install asks now, and a machine that already carries lerd has its mode read back out of the config, so the prerequisite check still knows whether this run needs certutil while the flag is left off entirely and the binary honours the choice it is meant to be reading.
- **A stop returned before the service had finished stopping** (#1510). Every stop enqueued the job and waited a fixed thirty seconds for systemd to report its result, so a service declaring a longer graceful window was guaranteed to outlast the wait. mysql and postgres both declare a sixty second stop timeout, which reaches the unit as `TimeoutStopSec=75`, while the client that asked for the stop gave up at thirty and returned as though the service had come down. The uninstall is where it left a mark: the stop returned early, the run deleted the unit files and reset their failed state, and podman then killed mysql a minute later, so systemd recorded a unit as failed after the reset that would have cleared it and left it behind on a machine that no longer had a lerd to clear it with. The wait comes from the unit itself now, read back as `TimeoutStopUSec` and given a margin so it outlasts the SIGKILL at the end of the window and leaves room for the container to be reaped, while anything that declares nothing, or less than the old thirty seconds, keeps them, so only a service that asked for a long shutdown waits longer.
- **Every `mysql` and `mysqldump` inside a PHP container died on a missing auth plugin**. MySQL 8.4 authenticates root with `caching_sha2_password`, and the client Alpine ships as `mysql-client` is the MariaDB one, which carries no such plugin, so `artisan migrate` on any project with a stored schema dump went down with it along with database imports and restores. This was handled once by naming `mysql_native_password` in the shared `lerd.cnf` authentication policy, but MySQL 9.x removed that plugin entirely and refused to initialise, so the line had to go and root fell back to the 8.4 default. Alpine's `mariadb-connector-c` ships `caching_sha2_password.so` in the client plugin directory, so the image installs it and the MariaDB client speaks the modern handshake against both MySQL lines while staying correct for MariaDB, with the fast path that layers over a pre-built base installing it behind a check for the plugin file so the apk becomes a no-op once a published base carries it.
- **`ftp_ssl_connect` did not exist in any published image** (#1576). ext/ftp only compiles its SSL half when OpenSSL is configured in the same run, and installing it through phpize against an already built PHP never sets that, so every image shipped an ftp without `ftp_ssl_connect` and any app doing FTPS died on an undefined function. The builder configures the extension explicitly before installing it, with the flag PHP expects for the version at hand since 8.4 renamed it, and `openssl-dev` joins the builder packages so the pkg-config probe can find the headers. `php -m` cannot see this class of bug because the module loads either way, so the base image workflow probes the function on the image it just built.
- **A declared extension the image already ships replaced it with a worse build of itself** (#1576). The image compiles its bundled extensions during its own configure run, with flags a standalone build layered on top cannot pass, and a declared custom extension that named one of them rebuilt it generically anyway. That is how `ftp` came back without OpenSSL and lost `ftp_ssl_connect` a second time, immediately after the base image had been fixed to ship it. Builds now skip any bundled name in the declared set and `php:ext add` refuses one up front rather than starting a rebuild that can only make the image worse. The fingerprint an image carries follows the set the build will actually realise, so an image built while a declared name was still replacing the better build reads as out of date exactly once and the next thing that touches the version puts the image's own build back, with nothing to run by hand.
- **`lerd domain add` doubled a name that already carried the TLD** (#1622). The TLD was appended to whatever it was given, so `shop.acme.test` was stored as `shop.acme.test.test` and served under a name no resolver would ever be asked for. The help says to pass the name without it, but the domain is the thing people have in mind and the thing they type, and remove had the same gap, so a mistyped add could not be undone by repeating it. Both subcommands now trim a trailing TLD through one helper, which is what the TUI's inline editor has always done.
- **A domain declared in `.lerd.yaml` was registered with the TLD twice** (#1631, #1632). A project's domains reached the registry through a path that appended the TLD unconditionally, so a project writing `main.test` in its `.lerd.yaml` was registered as `main.test.test`, an alias nothing resolves. `lerd domain add` learned to recognise a trailing TLD when the same thing was fixed there, and that helper is what the link path and the explicit name flag use now, so one spelling rule covers every surface. Accepting both spellings meant the writeback had to learn it too: the sync compared the project's own entries as raw strings against the registry's stripped ones, so `main` and `main.test` read as two domains and the file collected a duplicate pair on every link. Both sides go through the one key removal already matched on, in the CLI and in the dashboard's conflict-filtered path. Alongside it, a `.lerd.yaml` that does not parse is no longer discarded in silence: the resolver read the file with the error dropped, so a link on a broken one reported success with none of it applied, taking the name from the directory and the framework and PHP version from detection, and it stops with the YAML error and the path instead.
- **A launchd job outlived the service it belonged to** (#1622). Removing a container unit on macOS dropped the plist and stopped there, so launchd was never told the job was going and went on answering `launchctl list` for a path that no longer existed. That is where a job for a service disabled long ago was still coming from. The removal goes through the same stop funnel every other path uses, and Linux keeps the behaviour it had.
- **A test run rearranged the developer's own launchd** (#1622). The guard that keeps a test off the real service manager asked whether the unit lifecycle was unset, and on macOS the launchd manager is installed from an `init()`, so it never once fired: a single `go test ./...` booted out the running nginx job and re-registered it against a plist inside a temp directory the test then deleted. The manager a platform installs is now recorded apart from a stub a test assigns, so the refusal covers both, and a second guard sits under the `launchctl` helper because isolating `HOME` moves the plist but not the domain it is bootstrapped into. Three of the installer's checks could not run on a Mac either, reaching for `setsid` and spelling `script(1)` the util-linux way; that suite now passes 63 of 63 there instead of 60.
- **A debugging session was answered with a gateway timeout** (#1555). A request stopped at a breakpoint sends nginx nothing, and the vhost has always rendered a 60s `fastcgi_read_timeout`, so a minute into a session nginx answered 504 over a connection that was still perfectly alive. What the user saw was a gateway error rather than their breakpoint, and nothing anywhere reported a problem because from nginx's side there was none. The timeout knows about Xdebug now: a site whose PHP version runs a mode that includes step debugging renders 3600s instead of 60s, while a mode without debug in it and a version that is not the site's both leave the default alone, and an explicit `request_timeout` still wins since that is a deliberate answer to the same question.
- **A service could take the Xdebug port, and nothing reported it** (#1555). The port guard walks upward from a taken default and knew nothing about 9003, so a service whose own ports were busy could be published there; rustfs is the one that finds it, wanting 9000 and 9001 and landing on 9002 and 9003 where those are occupied. Nothing then failed: Xdebug connected back to the host exactly as configured, reached the container that took the port instead of the IDE, and the session died against a peer that does not speak DBGp, so the IDE simply never saw a breakpoint. The port the image is configured with is the port every allocator reserves now, and the ini is written from that same value so the two cannot drift apart. An install already published there keeps its port, since moving a service out from under whatever is pointed at it would be its own surprise, and doctor names the service holding it and the command that moves the right mapping, which is the part that was impossible to work out from the symptom.
- **A worker unit that answers to nothing was reported as needing healing** (#1577). Removing a worker from a framework definition removed it from lerd but not from systemd: the unit file stayed on disk, stayed armed for boot and stayed a unit the self-heal detector walked, so a worker nothing declared any more was reported as drifted and the dashboard drew a banner for it. Detection asks whether a unit's worker is still declared before it classifies the unit's state, counting a worker as declared when the framework definition names it, when the project declares it as a custom worker or when it is one of the builtins, and a gated worker counts too since a check that stops matching only hides it from the list. An install that can resolve none of this leaves every unit alone rather than calling all of them stale. What the definition left behind is reported once rather than removed silently, the way an orphaned vhost is: the site doctor names the units under Worker Units and its fix disables them, deletes them and reloads the daemon through the same teardown a worker stop goes through, since a unit may still be running something the user wants.
- **Workers were armed for boot while autostart was off** (#1531). Enabling a worker armed its systemd unit for login start no matter what the autostart switch said. Turning autostart off disabled every unit on disk at that moment, but anything written afterwards, a worker enabled in the dashboard or a unit rewritten by `lerd start`, went back to being wanted by `default.target`, so at the next boot systemd started it, `BindsTo` pulled the FPM container up with it, nothing pulled up the databases and caches behind it, and the worker died on a connection error and looped for as long as the session lasted. Arming goes through one place that consults the flag and disarms instead when it is off, which also clears an arming left behind by an older build, while the unit is still started by whoever asked for it so a worker that should be running now still runs. Two places also read "is the unit enabled" as "does the user want this worker", which that flag no longer answers once a running worker can be disarmed, so a PHP image rebuild snapshots which workers were up before it restarts FPM and brings exactly those back, and a node runtime switch regenerates the unit of a worker that is merely running.
- **A worker that was down when another one was touched stopped coming back** (#1627). The workers list in a site's `.lerd.yaml` is what `lerd start` brings back, and it was rewritten after every worker action as the set of workers running at that moment, so a worker that was merely down when an unrelated one was started or stopped was written out of the file with nothing to put it back. A crash-looping worker is down by definition, which is how one disappeared for good: any worker action taken while it was failing dropped its declaration, and disarming those units at boot then removed the accident that had been starting it. The list is a declaration, so it follows the units on disk now, written when a worker starts and removed when it stops, and the running set is still consulted for stripe, the host-proxy dev server and orphans, which have no unit of their own.
- **`lerd stripe:listen` could stop the listener instead of starting it** (#1609). The stop command was declared as `stripe:listen stop` and registered on the root next to the start command, and since cobra takes a command name from the first word of that, the root carried two commands both called `stripe:listen`. Root help printed the entry twice, once as the starter and once as the stopper, and the plain form resolved to whichever of the two cobra reached first, which on a current build is the stopper, so the documented start with its `--path`, `--api-key` and `--secret-env-key` landed on a command that accepts none of them and tore the listener down. Stop hangs off the start command as a plain `stop` child now, the shape the generated worker commands already use, so `lerd stripe:listen` reaches the starter, `lerd stripe:listen stop` keeps working, and the root lists the entry once. This has been there since the listener became a systemd service in 0.6.0.
- **A worker proxy could only serve one path** (#1571). A worker proxy named a single path, which is fine for a server that listens on one and wrong for every server that does not. Reverb is the case that surfaced it: its socket lives on `/app` and the HTTP broadcasting API on `/apps`, so a broadcast fired from the application fell through to PHP and returned a 404 while the socket itself worked. The definition takes a `paths` list and the vhost renders one location per entry, on the same port and with the same normalising and escaping the single path always had, and where both are set the list wins so a definition can carry the old key and still proxy on binaries released before this.
- **Logs were unreachable for a worker whose name carried an underscore** (#1527). The log stream route validated both the site and the worker segment of `/api/worker/{site}/{worker}/logs` against a lowercase alphanumeric and hyphen pattern, while custom worker names in `.lerd.yaml` are free-form apart from whitespace, so a worker declared as `all_queue` never matched: the request 404'd before any stream opened and the UI sat on Disconnected with a Reconnect button that could only 404 again, while the worker ran the whole time. The same gate applied to the site segment, so a site handle carrying an underscore lost every worker log pane for the same reason. Underscores and capitals pass now, while dots, slashes and shell metacharacters still do not, so a unit name can never walk out of the log directory it is read from.
- **An Expose tunnel was named after an ephemeral port number** (#1551). Expose derives a tunnel's subdomain from the host it is pointed at, and every `lerd share` is pointed at the local rewrite proxy on loopback, so a share asked for the proxy's ephemeral port number as its public name and died as soon as another user held it. The site's own label is requested explicitly now, so a share is named after the site it serves and a worktree after its branch; custom subdomains are an Expose Pro feature, so a free account is told the request was ignored and gets a random name, which still leaves a working tunnel rather than a collision on a number nobody chose. The dashboard also missed a tunnel tool sitting in lerd's own bin directory, which is where a composer global install of Expose lands and which reaches a shell through `lerd path:enable` but never reaches a daemon, so it joins the Homebrew prefixes the daemons already search and `share:tool` resolves a tool the same way instead of consulting PATH alone.
- **A failed dnsmasq build left the install reporting success** (#1537). The dnsmasq image is the one thing lerd builds rather than pulls, and the `apk add` inside it resolves names from the build's own network namespace rather than through the host resolver that pulled the base image a second earlier, so on a host where podman does not translate the stub resolver into something that namespace can reach, every pull succeeds and the build alone fails with a DNS transient error. The build retries once with the host's real upstream nameservers pinned, which is the only lever available since a rootless build cannot attach to the lerd network, and the three copies of it that had drifted apart across install and start are one helper now. When both attempts fail the install says so at the end instead of reporting a clean finish, because without the image lerd-dns cannot start at all and the previous behaviour left the user with a completed install, a unit restarting in a loop and nothing connecting the two. Pinned nameservers do nothing for a namespace where none of them is reachable at all, which is what the reported machine turned out to have, so a last attempt drops it and builds on the host network, the one that demonstrably resolved the base image seconds earlier.
- **The dashboard, tray and TUI read a DNS suffix nothing served** (#1559). Three readers still took `dns.tld` raw and probed it, the dashboard's DNS status watcher, the status snapshot the dashboard and tray render, and the TUI system pane, so on a config the writer refused they hunted for a suffix nothing ever served and DNS read as down in all three while the CLI reported it working. The status snapshot also dropped the default when the config loaded with an empty tld, probing a bare dot instead of `test`.
- **A PHP version installed from the dashboard kept showing its bare minor** (#1587). The patch number is probed out of the image and cached per version, and only the rebuild handler re-probed it before reporting the build done, so the status load the client makes when the modal closes found nothing cached for a version that had never been probed and the card stayed as it was until the page was reloaded. Both build handlers go through the same refresh, which polls the container states, probes the patch out of the image that was just built and drops the status snapshot before the done event.
- **The doctor's database fix was undone by its own cache** (#1649). The fix created the schema, said so, and then the re-check underneath it reported the same database as still missing and offered the same remedy, so anyone reading the two lines in order concluded the fix was broken while a plain run a moment later passed. An engine's contents are read through a list memoised for five seconds, so that a sweep across every linked site does not pay a container exec per site to ask the same engine the same question, and the re-run of the checks happens in the same process and so always inside that window. Creating a database drops that engine's entry now, scoped to the engine that changed so a sweep across sites on different services does not pay for every create, and the dashboard keeps its own copy of the fix and gets the same call, where it matters more, since lerd-ui outlives the request and a stale entry would answer the next check as well as this one's.
- **An update drew a failed step for every built-in service** (#1517). The preset refresh that runs on install and update fetched the store definition for every installed service, built-ins included, and those ship embedded in the binary and were never published to the service store, so each came back a 404 and was drawn as a failed step in the middle of an otherwise clean update. The refresh reads the store index first and keeps only the presets the store actually publishes, which also makes the header count match what is about to be fetched, and when the index cannot be reached there is no way to tell a published preset from a built-in, so it does nothing at all and leaves the cached and embedded copies serving.
- **A build run from a scratch directory claimed the shims and the daemons** (#1603). `LerdBinary` returns the running executable, and that path goes into every shim in lerd's bin directory and into the systemd units, so a build run out of a temporary directory claimed all of them and was then deleted, leaving the shims alive only on their `LERD=lerd` fallback and the daemons failing with a bare exit status 203 until the next start repaired them, recording whatever scratch path happened to be running by then. A binary under a temporary directory is not an install, so it falls back to the install location, the same one `LerdBinary` already returned when the running executable cannot be resolved at all.
- **A passthrough child did not own its stdout** (#1602). `lerd php` and `lerd console` wire the child's stdout straight through, which is what lets them stand in for `php`, and a framework's own local MCP server is started exactly that way and speaks JSON-RPC on that same stream, so the paused site header, the per-service starting line and the failed start warning were landing in the middle of it. They go to stderr now, where the FPM spinner already was for the same reason.
- **Headed Pest browser tests had nothing to draw on** (#1538). A headed launch inside the FPM container has no display, and Playwright reacts by refusing to start and printing a box telling you to run an XServer or wrap the app in `xvfb-run`, which reads as advice for the host rather than for the container the tests actually run in. `pest:browser install` bakes Xvfb into the image alongside chromium and the shim gives a headed launch a virtual display, while headless keeps execing chromium directly since Playwright appends `--headless` itself and that flag is the only signal the shim gets about which mode it was launched for. The window lives on a virtual screen, so screenshots see what the browser saw and the Selenium preset with its noVNC dashboard remains the way to watch a session.
- **The headless Playwright binary was never shimmed** (#1539). The `chromium_headless_shell` build ships its executable as `headless_shell`, a name the shim never looked for, so it stayed the glibc binary Playwright downloaded and musl could not exec it, while the count guard did not notice because chrome still matched and the count stayed above zero, so the install reported success while shimming half of what it downloaded. The failure surfaced as `PlaywrightOutdatedException`, which reads as a stale Playwright and sends people to reinstall it, so the wrong half of the setup got blamed. The extractor learns the same name too, otherwise the binary arrives without its executable bit.
- **An empty config file broke a project that was only waiting to be installed** (#1563). A file the application writes during its own install was created empty when missing, the same way lerd creates a dotenv it owns, and to a framework an empty one is not a blank slate but a claim that it is already configured, so a TYPO3 project serving its installer returned 500 the moment lerd made the file. The doctor turned that into a trap rather than an accident, reporting the file as missing and naming `lerd env` as the way to create it, so following the advice is what broke the project. The write is skipped with a line saying why and the doctor points at the framework's own install instead, while the file the definition names as the plain one carries the database and service wiring until the installer has written the app one, so the setup a project needs still happens on the way through. Magento sits in the same position with `app/etc/env.php`, so this covers it too.
- **Sites that could run vite had no dev server worker**. The vite worker had only ever been added to Laravel 11 and up and to Tempest, and nothing noticed that every other definition able to run vite was missing it, so a site on one of those got no dev server worker at all and the wrapper config that puts vite behind the site domain was never generated. A check now walks the store checkout and requires the worker on each framework from the major where vite became its asset pipeline, so a future Laravel or Statamic version inherits the requirement instead of quietly repeating the gap, and any definition declaring the worker is held to the shape the dev server machinery depends on, since the wrapper writer and the nginx proxy both assume it runs on the host under a check for `node_modules/vite`.

### Docs

- The dashboard screenshots eighteen pages had been pointing at are in the repository. The web UI, dumps, tinker and worktree pages named files that were never committed, and the docs workflow only ever copied the three landing page captures, so every one of them had been a broken image on the site since May. They are captured from the backend-free UI demo, which is the same web UI running on sanitized fixtures, so the pages show real interfaces without a single real project name. Two of them documented a UI that no longer exists, since the site detail lost its inline branch picker and its worktree management modal to the tab strip a while ago.
- A TYPO3 walkthrough, from create-project to a served site with MySQL, Mailpit and the scheduler, covering the two things specific to it: the installer differs by major, and the config file belongs to TYPO3 rather than to lerd, so an empty one breaks a project that was only waiting to be installed. The per-major table carries the PHP range and the config path since both move across the five versions, and the older majors get their own note, since composer refuses create-project for 10 to 12 while their releases carry advisories.
- Omarchy, Laradock and Sail landing pages. Omarchy ships a finished desktop with no web stack at all, so the page covers the Arch prerequisites that are not defaults, the systemd-resolved without NetworkManager case, why the tray autostarts there, and the bar widget. Laradock had no page despite being the thing a lot of people are trying to leave, and since there is no automated importer for it the page documents the manual path. Both it and the Sail page now open on the thing that is true only of them rather than on the shared-infrastructure argument every comparison page here already makes. The Getting Started sidebar is grouped into Start here, Platform guides and Coming from another tool, with the existing labels kept so nothing already indexed changes.
- The Omarchy bar widget has a page of its own, covering what lerd Glance reads, the two buttons that can change anything, and where it stops, so nobody has to open the plugin's repository to find out whether it talks to anything but the local dashboard.
- Why an Xdebug session stops inside lerd's dump bridge. lerd mounts an `auto_prepend_file` into every PHP version it builds, so it is the first file a request runs, and an IDE told to stop on the first line of every script stops there rather than in the application, in a file that exists only inside the container. The page names the setting in PhpStorm and its VS Code equivalent, along with the two under External connections that are on by default and both match the bridge, and points at on-demand debugging for the half of this that is `start_with_request` connecting every request before anyone asked to debug one.
- The reference gaps 1.33.1 left. Everything merged since shipped documentation with it, but the tray toggle, the nginx command family and the PHP 8.6 tier never reached the pages a reader looks things up in, and eight links pointed at anchors that do not exist. The reference AI assistants read names the 8.6 tier, `lerd cpx`, the store's package layer and the tray toggle, and tells assistants what the doctor and the tool list actually do now, since it still described three host-side site_doctor fixes and still called doctor an environment check.
...
v1.31.0 a v1.33.1. Nenhuma feature nova do fork nesta versão: as que existiam
foram preservadas, e o resumo abaixo cobre o que **mudou de comportamento** na
integração.

São três ciclos do upstream de uma vez. O 1.32.0 termina a história de expor um
site, começada no 1.31.0: share público por um wildcard próprio, respondido pelo
reverse proxy que a máquina já roda e com o vhost comum do site intacto ao lado;
Pinggy entrando como linha de tabela em vez de mais um caso especial; acesso LAN
aos engines gerenciados como opt-in; e as ações do dashboard que alcançam o host
atrás de um switch próprio. O 1.33.0 é a segunda lei de design cobrada até o
fim: o card de bancos, o doctor, o `lerd env` e o `mail()` liam a estrutura de
um projeto Laravel e por isso erravam em Drupal, WordPress e Symfony, e cada um
passa a tirar da declaração do próprio framework qual arquivo ler, em que
formato, em que chaves e com que comando. O 1.33.1 é patch, e três deles valem
para esta máquina: o lerd deixa o resolver do host em paz onde existe
`/etc/NIXOS`, o uninstall remove via `podman unshare` os arquivos que os
containers escreveram como subuid, e o pgAdmin para de reiniciar a cada
`lerd start`.

### Changed

- **As rotas que o fork fechava por completo ganharam um escape opt-in.** O gate
  loopback-only do fork negava a clientes remotos, sem exceção, as rotas que
  agem sobre o host, entre elas `/api/services/{name}/env`, que devolve o bloco
  `Environment=` do container com `ORACLE_PASSWORD` dentro. O upstream 1.32.0
  introduziu o switch `remote_full_access` e o comando
  `lerd remote-control full-access on`, e a integração adotou esse escape: a
  condição do gate passa de `isLoopbackOnlyPath(path)` para
  `isLoopbackOnlyPath(path) && !cfg.UI.RemoteFullAccess`. O default continua
  fechado, um config que não carrega é lido como fechado, e a checagem continua
  vindo antes das credenciais, então uma instalação que não optou responde 403
  igual a qualquer palpite. Ligar o switch expõe essas rotas de propósito, e no
  fork a lista é maior que a do upstream: inclui `/api/proxies`, `/api/debug` e
  os subcaminhos `/env` de sites e de serviços.
- **O `lerd env` foi reescrito em volta do ramo Oracle, que não mudou.** A
  detecção de banco do 1.33.0 lê a declaração do framework (`fw.Env.SQLite`,
  `dbConnectionKey`) em vez de presumir as chaves de `.env` que Laravel usa.
  Oracle não é preset e não tem serviço em container, então segue entrando pelo
  bloco `oracle:` do `.lerd.yaml` com as mesmas condições de antes; o que mudou
  é o fluxo em volta dele.

### Preservado

Verificado arquivo por arquivo depois da integração: o filtro de comandos
destrutivos nos defaults (`migrate:fresh`, `db:wipe`, `migrate:reset`,
`migrate:rollback` e `doctrine:fixtures:load` seguem fora dos one-clicks), a
validação anti-traversal de nome e versão de framework, a rejeição de valores
perigosos nos snippets de nginx, a allowlist SSRF do `lanshare`, o Oracle como
banco externo no `init` e no `env`, o `TNS_ADMIN` montado somente leitura para
`tnsnames.ora` e wallet do Autonomous, e as chaves de i18n próprias do fork nos
14 idiomas, sem perder nenhuma chave ou tradução do upstream.

---

## [1.33.1] - 2026-08-14

A patch on top of 1.33.0, and the reason to ship it quickly is the Homebrew one, since a repair only helps the machine that already carries it. 1.33.0 stopped lerd writing the keg path into its own units and shims, but an install set up before that still names one, and every half of the repair waits for a person to show up: the units are rewritten by the next `lerd start` and the environment by the first command run at a terminal. On a machine that upgrades its packages unattended neither ever happens. There is a second reason to ship it quickly, which is that on NixOS lerd had been taking the whole machine's DNS down since 1.30.0, quietly enough that the recovery advice in its own guide stopped working a release later. The rest is mostly the uninstall, which failed on the data a service wrote, said it had purged images it had kept, and left a binary, a cache directory and a PATH entry behind on a machine it had just told the user was clean. Then a site that could not be linked again after being unlinked, the SSL vhost every generator writes and only some of them installed, and the tray's Services menu disappearing on its own. Two more come off the same file podman re-owns out of the user's reach: pgAdmin restarted itself on every start because the rendering could not be read back to see that it had not changed, and a removed service left that rendering on disk for the next install of the same preset to inherit.

### Fixed

- **lerd's DNS setup took down all name resolution on NixOS** (#1511, #1512). NixOS generates `resolved.conf` from `configuration.nix`, where the documented setup routes only `~test` to the lerd-dns container, and lerd's own resolver work fights that from two directions at once: since 1.30.0 it writes the always-up `lerd0` link and empties systemd-resolved's `FallbackDNS`, and since 1.31.0 the bootstrap step installs a passwordless DNS sudoers rule, so a start or a watcher repair re-applies both without asking. What the user sees is every name failing, not just `.test`, on a machine where nothing else changed, and the guide's advice to press Ctrl+C at the dispatcher prompt no longer has a prompt to interrupt. lerd leaves the host resolver alone now wherever `/etc/NIXOS` exists, across the resolver config, both sudoers paths and the bootstrap grant; the lerd-dns container still runs and the `~test` route in `configuration.nix` is what carries `.test`, and every other distribution keeps the path it had. Skipping quietly would trade one silent failure for another, so install and start say once that they are leaving the resolver alone, and the diagnostic reports the hookup as skipped rather than failed, drops the interface and `lerd0` rungs that probe files lerd no longer writes, and points a failed lookup at `configuration.nix` rather than at the `lerd install` that is now guaranteed not to help.
- **A Homebrew upgrade left the install broken with nothing on screen to say why** (#1432). `brew upgrade lerd` retires the keg that an older install's user services and PATH shims still name, so the dashboard and the watcher stop with a bare exit status 203 and `php` resolves to a file that is gone, on a machine where the developer changed nothing. The formula runs a `post-upgrade` hook now that repoints the daemon units and the shims at the lerd that is installed and restarts the daemons set to start at login, and stops there, because reapplying the quadlets, nginx and the stores takes minutes and belongs to the first command run at a terminal, where someone is watching. It does nothing on a machine that has never run `lerd install`, since that install asks questions no package script can answer, and it cannot fail the upgrade. What counts as a stale path now depends on who asks: a start still repairs only a binary that is no longer on disk, so a build run from a checkout cannot claim a working install's daemons, while the hook runs from the release that has just landed and so also repoints an older keg of the same formula while brew still has it, since the cleanup that deletes it comes later. brew runs `post_install` inside a read-only mount namespace with `HOME` pointed at a throwaway directory, where every write into the real home fails quietly, so on Linux the repair is handed to the user's own systemd manager, which runs it outside that namespace.
- **The uninstall could not remove the data the services had written** (#1489, #1280). Containers write their files as a subuid inside the rootless user namespace, so mysql, postgres, redis and the rest leave trees under `~/.local/share/lerd` that a plain removal cannot touch. The installer's `--uninstall` ran that removal under `set -e`, so the first tree it could not delete took the script down before any success line, by which point the binary and the units were already gone and the user was left holding a directory they could not remove and nothing to help them do it. Both the installer and `lerd uninstall` retry through `podman unshare`, which enters the namespace where the files are removable, and neither reports a removal that did not happen any more: when even that cannot clear the directory the uninstall names the one that survived and prints the command to run by hand.
- **The uninstall said it purged images it had kept, and left three things on disk either way** (#1505). It lists the images with <code v-pre>podman images --format {{.Repository}}:{{.Tag}}</code>, which spells a local build `localhost/lerd-dnsmasq:local`, and matched that against a name with no registry on it, so nothing ever matched and every image lerd built itself survived an uninstall that announced it was purging them. The matcher was covered by a test that fed it the bare spelling podman never produces, which is how it went unnoticed. Three more survivals go with it: `~/.cache/lerd` was written by the registry cache and removed by neither path, and the installer's `--uninstall` removed `lerd` while leaving `lerd-tray` beside it along with its running unit, and cleared only the PATH line it had written itself while the one lerd writes from inside the binary stayed, pointing at a directory that was being deleted in the same run. Units stopped by deleting their files are also reset now, rather than lingering in `systemctl --user` as failed and not-found on a machine with no lerd on it. And the guard that keeps a package-managed binary where it is, so brew or apt still believes lerd is installed, was declining to remove any binary at all, which on a machine carrying both a packaged install and a script one left `~/.local/bin/lerd` on PATH pointing at a data directory the same run had deleted. That pair is lerd's own to remove whatever the running binary is; only the packaged one is left alone.
- **The tray's Services submenu came and went on its own** (#1503). The whole section disappeared from the menu every so often, most reliably right after starting or stopping a service, and came back at the next poll. `/api/services` is rebuilt from a subprocess per installed service, so a cold rebuild takes several seconds, and the snapshot it is served from holds for five minutes when no dashboard tab is open, which makes the tray the poller that pays for it. Its HTTP client gave up after four seconds, so that poll came back with nothing while `/api/status` answered in a millisecond and kept reporting the environment as running, and the menu drew the empty result as an install with no services and hid the section. The tray waits twenty seconds now, which the poll every thirty can afford, and a services call that does not come back leaves the section exactly as it was rather than emptying it. The submenu still hides when there genuinely are none, including when lerd is stopped, since then the emptiness is an answer.
- **A site that had been unlinked could not be linked again, and a generated SSL vhost was left beside the served one** (#1496). Unlinking a site under a parked directory keeps its registry entry and marks it ignored, which is what stops the watcher adding it straight back. Linking it again compared the plan against that entry, found the two identical and concluded there was no work to do, so it printed "already linked" with an https address that answered nothing while the entry stayed ignored and no vhost was written. An ignored entry serves nothing, so it never counts as the registration a link would write. Separately, every SSL vhost generator writes a `<domain>-ssl.conf` the caller is expected to displace `<domain>.conf` with, and that rename was open coded in five places and skipped in three, so the FPM runtime switch, the reverb vhost regeneration and the MCP site runtime action each left one behind; `conf.d` is a glob and the sidecar sorts first, so nginx served it and ignored the file every other path rewrites, and once `lerd unsecure` deleted the certificate it pointed at, nginx refused to load the configuration at all rather than only that site. Installing the sidecar is one call all of them share now, and the vhost repair reads a sidecar as the site it belongs to, so a half-installed render can be repaired rather than only deleted as an orphan. The repair only ever looked at a vhost whose certificate was missing, since those are the ones that stop nginx loading at all, so a sidecar left by a build from before this went on being served indefinitely; `lerd start` now finishes that render by installing it over the file it was meant to replace, leaving alone the site that is paused or ignored, because there the vhost being served is the one lerd swapped in on purpose. The repair also decided who owned a vhost by asking the registry whether the site was secured (#1506), so a site whose entry and whose file disagreed, which is what a half-finished unsecure leaves, was read as belonging to nobody and had the only vhost serving it deleted. Ownership is the domain alone now: a registered site gets its vhost re-rendered over plain HTTP, and only a domain no site holds is removed.
- **pgAdmin was stopped and started again on every `lerd start`** (#1490). Each start reported that it had applied an updated config and restarted the container, on an install where nothing had changed, so anything holding a pgAdmin session lost it and a line meant to be rare became one to scroll past. A service is restarted when the newest of its rendered files is newer than the container's boot, and pgadmin's `pgpass` is declared `chown: true`, which is podman's `:U` re-owning the file to a userns-mapped uid; the host user can no longer read it back, so the compare that decides whether a rendering actually changed could not run and fell through to a rewrite on every pass, landing an mtime a couple of seconds past the restart that pass had just performed. The next start read that as drift and restarted again, indefinitely. A sha256 of the rendering is kept beside the file now, never mounted so podman never re-owns it, and answers the question when the file itself cannot be opened, which leaves the mtime where it is when nothing changed and still reaches a file that has to be rewritten. It follows the mount that needs it rather than sitting beside every rendered file, since a mount lerd can still read compares by content the way it always did.
- **Removing a service left the config its preset had rendered on disk** (#1507). The remove stops the container, deletes the quadlet and the service YAML and reports the service gone, while the directory under `service-files` stayed exactly as it was with nothing in the output to mention it. So a remove followed by a reinstall of the same preset inherited whatever the previous install had written rather than starting from a clean rendering, including a file podman had re-owned past the point the user could read it, and a directory sat there named for a service that no longer existed. Those files are derived from the definition being deleted rather than from anything the user put there, so they leave with it, while the data directory and the tuning override keep the behaviour they had and are still the user's to lose on purpose. That only helps from here on, so cleanup reclaims the directories an older lerd left behind, listing one only when nothing answers for the name any more, no definition, no default preset and no installed quadlet, and showing it in the plan alongside the images with its own size. The plan is no longer images only, so the command's header, column and count say so, and the dashboard modal follows in every language.

### Docs

- The landing page draws each framework and service with its own mark (#1498). Both stores publish that artwork and the dashboard already renders it, so the page uses the same SVGs tinted with each brand's colour rather than a lettered tile, keeping the tile only for RustFS, which has no symbol mark published anywhere. The framework strip links to its walkthroughs instead of sitting there as nine dead spans. The tile's outline sat on a rect filling the viewBox, so half its width fell outside and every icon on the page was clipped on all four edges.
- The demo behind the landing page offers the database actions a real install does (#1500). Its fixtures were captured before `supports_export`, `supports_import` and `supports_drop` existed, so every card rendered with copy and snapshot while the daemon has been sending all five flags for some time.
...
- `oci8`, `amqp` e `memcached` agora constam em `podman.BundledExtensions`. As
  três já vinham compiladas em toda imagem do fork, mas não eram anunciadas, então
  `lerd park` nunca avisava que um projeto precisava delas.
- Traduções das 4 chaves de UI do fork (`nav_proxies`, abas de log do PHP) nos 6
  idiomas que ainda não as tinham (it, ja, pl, ro, vi, zh).
- `LERD_REPO` (do upstream) sobrescreve o repositório de releases no instalador;
  o default continua sendo o fork.

### Changed

- **Store de frameworks/serviços aponta para o upstream.** O upstream centralizou
  todas as URLs próprias no pacote novo `internal/origin` e migrou a org
  `geodro` → `lerd-env`. No fork, releases/changelog/imagens GHCR continuam no
  fork; frameworks e serviços passam a apontar para `lerd-env`, porque o
  `gabriel-sousa99/lerd-frameworks` que o fork referenciava **não existe** (404) —
  o store estava quebrado.
- Gerenciamento de extensões PHP pelo dashboard migrou para a aba
  `PhpExtensionsTab` do upstream, e o conjunto declarado passou a ser **global**
  (upstream trocou `map[versão][]ext` por uma lista única aplicada a toda imagem).
  O `{version}` da rota agora escolhe qual imagem é reconstruída, não quais
  extensões são editadas.
- `GET /api/php-versions/{v}/extensions` passa a servir o relatório do upstream
  (declarado × realmente carregado); o endpoint declarado-apenas do fork, que
  sombreava essa rota, foi removido. `POST`/`DELETE` seguem sendo do fork.
- Bloco "Proxy fullstack" migrou de `SiteDetail` para `SiteOverview`, seguindo a
  extração de componentes do upstream.

### Fixed

- **HTTPS sem DNS gerenciado voltou a funcionar.** O upstream introduziu cinco
  novos pontos que barram HTTPS quando `dns.enabled: false` — `siteops.SetSecured`,
  `siteops.ResolveSecured`, `EnforceGroupSecondaries`, o toggle da TUI e o cadeado
  do dashboard. Como o fork instala a CA do mkcert em qualquer modo e `.localhost`
  resolve por RFC 6761, todos foram removidos: caso contrário o modo **padrão** do
  fork perderia HTTPS.
- `SyncEnvIfPrimaryChanged` (novo no upstream) agora é ciente do proxy fullstack:
  o `.env` do site continua apontando para o domínio unificado em vez de ser
  sobrescrito com o domínio próprio do site.
- `lerd about` e o cabeçalho do `install.sh` mostravam o **próprio fork** como
  upstream; agora apontam para `lerd-env/lerd`.
- `mkdocs.yml` apontava para `geodro.github.io`, morto após a renomeação da org.
- `testTimeout: 30000` no vitest. O upstream levou as mensagens de i18n a 1159
  chaves; com as do fork são 1163 × 14 idiomas = bundle paraglide de ~8,6 MB em
  1167 módulos. O primeiro `await import()` de cada suíte de store paga esse
  transform (~5 s sozinho, mais sob 16 workers), estourando o default de 5 s em
  12 suítes mesmo com a máquina ociosa. Os testes em si são rápidos.
...
<!--
  Abaixo: histórico do upstream (lerd-env/lerd), preservado na íntegra. As
  entradas do fork ficam acima, no esquema vX.Y.Z-oracle.N.
-->
