...
func TestNSSDBDirsOnlyReportsStoresMkcertWritesTo(t *testing.T) {
	...
	dirs := nssDBDirs()
	if !slices.Contains(dirs, native) {
		t.Fatalf("nssDBDirs() = %v, want native profile %s", dirs, native)
	}
	if slices.Contains(dirs, flatpak) {
		t.Fatalf("nssDBDirs() = %v, must not include Flatpak profile %s", dirs, flatpak)
	}
	for _, dir := range dirs {
		if dir != native && dir != "/etc/pki/nssdb" {
			t.Fatalf("nssDBDirs() = %v, unexpected store %s", dirs, dir)
		}
	}
}
