...
// Upstream refuses to secure a site when lerd-managed DNS is off. The Oracle
// fork deliberately does not: `lerd install` puts the mkcert CA in the trust
// store regardless of the DNS choice, and .localhost resolves to the loopback
// via RFC 6761, so HTTPS is reachable on both the .test and .localhost paths.
// Whatever else SecureSite does here, it must never refuse with ErrDNSDisabled.
func TestSecureSite_DoesNotRefuseWhenDNSDisabled(t *testing.T) {
	tmp := t.TempDir()
	...
	site := config.Site{Name: "myapp", Path: filepath.Join(tmp, "myapp"), Domains: []string{"myapp.localhost"}}
	if err := SecureSite(site); errors.Is(err, ErrDNSDisabled) {
		t.Fatalf("SecureSite refused with ErrDNSDisabled; the fork serves HTTPS without lerd-managed DNS")
	}
}
