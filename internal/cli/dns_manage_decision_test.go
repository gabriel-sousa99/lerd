...
func TestDNSManageDecision(t *testing.T) {
	cases := []struct {
		...
		// Only a genuine first install (no config file) asks. The Oracle fork
		// defaults that prompt to *.localhost rather than managed DNS.
		{"first install asks", false, false, nil, true, false, true},
	}
