...
func TestUserPickedDBFromYAML(t *testing.T) {
	for _, tc := range []struct {
		...
		{"oracle builtin", map[string]bool{"oracle": true}, true},
...
`
	...
	if !nameSet["sqlite"] || !nameSet["mysql"] || !nameSet["postgres"] || !nameSet["oracle"] {
		t.Errorf("buildDatabaseOptions must always include the built-in trio plus oracle, got %v", nameSet)
	}
...
func TestShouldApplyService(t *testing.T) {
	for _, tc := range []struct {
		...
		// Oracle behaves like built-in DBs: skip when another DB was picked.
		{"oracle picked, not detected", "oracle", false, true, true, false, true},
		{"oracle detected, mysql picked", "oracle", true, false, true, false, false},
