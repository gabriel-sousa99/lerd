...
func runLink(args []string) error {
	...
	if servesPHP {
		...
		// Runs before the framework's required services are folded in, so the
		// gate below reads the project's own committed choice.
...
func promptLinkDatabaseChoice(cwd string, proj *config.ProjectConfig) *config.ProjectConfig {
	...
	dbFramework, _ := config.GetFrameworkForDir(proj.Framework, cwd)
	dbOptions, _ := buildDatabaseOptions(dbFramework)
