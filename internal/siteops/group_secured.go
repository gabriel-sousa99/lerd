...
//
// Upstream skips the whole pass when lerd does not manage DNS, on the premise
// that nothing has HTTPS there. That premise does not hold in the Oracle fork,
// where .localhost sites are secured too, so the invariant is enforced either
// way — otherwise a secured main's wildcard would keep swallowing its plain-HTTP
// secondaries on exactly the install shape the fork defaults to.
