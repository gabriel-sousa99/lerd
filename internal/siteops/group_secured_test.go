...
// Upstream no-ops the whole pass with DNS off, on the premise that nothing has
// HTTPS there. The Oracle fork secures .localhost sites too, so a secured main
// still publishes a 443 wildcard that would swallow a plain-HTTP secondary's
// subdomain — the invariant has to be enforced on exactly this install shape,
// which is the fork's default.
func TestEnforceGroupSecondaries_stillEnforcesWhenDNSDisabled(t *testing.T) {
	stubs := groupSetup(t)
	...
	if len(changed) != 1 || stubs.secureCallCount != 1 {
		t.Errorf("changed=%v secureCalls=%d, want the secondary secured with DNS disabled", changed, stubs.secureCallCount)
	}
}
