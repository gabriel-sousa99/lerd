...
// Upstream refuses to secure a site when lerd does not manage DNS. The Oracle
// fork installs the mkcert CA regardless of the DNS choice and .localhost
// resolves to the loopback via RFC 6761, so securing works on a DNS-off install
// — which is the fork's default, making this the common path, not an edge case.
func TestSetSecured_securesWhenDNSDisabled(t *testing.T) {
	stubs := stubSecureDeps(t)
	projectDir := withTempEnv(t)
	...
	gcfg.DNS.TLD = "localhost"
	if err := config.SaveGlobal(gcfg); err != nil {
		t.Fatalf("save global: %v", err)
	}

	site := &config.Site{Name: "myapp", Domains: []string{"myapp.localhost"}, Path: projectDir}
	if err := config.AddSite(*site); err != nil {
		t.Fatal(err)
	}
	...
	if stubs.secureCallCount != 1 {
		t.Errorf("certs.SecureSite calls = %d, want 1 with DNS disabled", stubs.secureCallCount)
	}
	if !site.Secured {
		t.Errorf("site.Secured did not flip with DNS disabled")
	}
}
